...
function addMissingFields(section: TemplateSection, fields: TemplateField[]): TemplateSection {
...
export function enhanceTemplateSections(studyType: StudyType, rawSections: unknown): TemplateSection[] {
  if (!Array.isArray(rawSections)) return []

  const sections: TemplateSection[] = rawSections
    .filter((item): item is TemplateSection => typeof item === 'object' && item !== null && 'id' in item && 'type' in item && 'title' in item)
    .map((section) => ({
      ...section,
  ...
  const respiratoryExtra: TemplateField[] = [
    ...
    { key: 'longest_apnea_sec', label: 'Durée apnée la plus longue', unit: 'sec' },
    { key: 'cheyne_stokes_index', label: 'Index de Cheyne-Stokes', unit: '%' },
    { key: 'snoring', label: 'Ronflement', unit: '' },
  ]

  const respiratoryIndex = sections.findIndex((section) => section.id === 'respiratory' && section.type === 'metrics')
  if (respiratoryIndex >= 0) {
    sections[respiratoryIndex] = addMissingFields(sections[respiratoryIndex], respiratoryExtra)
  }
  ...
  if (!hasSection(sections, 'oxymetry')) {
    sections.splice(Math.max(sections.length - 1, 1), 0, {
      ...
      type: 'metrics',
      fields: [
        { key: 'spo2_base_awake', label: 'SpO2 de base au repos (éveil)', unit: '%' },
        { key: 'time_under_88', label: 'Temps passé sous 88%', unit: '%' },
        { key: 'baseline_stability', label: 'Stabilité ligne de base', unit: '' },
      ],
    })
  }
