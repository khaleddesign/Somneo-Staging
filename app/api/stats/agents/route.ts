...
export async function GET() {
  try {
    ...
    if (completedAllError)
      return NextResponse.json(
        { error: completedAllError.message },
        { status: 500 },
      );
    ...
    for (const row of inProgressStudies || []) {
      ...
      inProgressByAgent.set(
        ...
        (inProgressByAgent.get(row.assigned_agent_id) || 0) + 1,
      );
      if (row.updated_at) {
    ...
    for (const row of completedThisMonth || []) {
      ...
      completedByAgent.set(
        row.assigned_agent_id,
        (completedByAgent.get(row.assigned_agent_id) || 0) + 1,
      );
      if (row.updated_at) {
        const current = lastActivityByAgent.get(row.assigned_agent_id);
        if (
    ...
    for (const row of completedAll || []) {
      if (!row.assigned_agent_id) continue;
      completedTotalByAgent.set(
        row.assigned_agent_id,
        (completedTotalByAgent.get(row.assigned_agent_id) || 0) + 1,
      );
      ...
      if (row.submitted_at && row.completed_at) {
        ...
        avgByAgent.set(
          row.assigned_agent_id,
          (avgByAgent.get(row.assigned_agent_id) || 0) + duration,
        );
      }
    ...
    const perAgent: AgentKpi[] = (agents || []).map((agent) => ({
      ...
      avg_turnaround:
        (completedTotalByAgent.get(agent.id) || 0) > 0
          ? Math.round(
              ((avgByAgent.get(agent.id) || 0) /
                (completedTotalByAgent.get(agent.id) || 1)) *
                10,
            ) / 10
          : 0,
