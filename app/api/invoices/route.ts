...
export async function POST(req: NextRequest) {
  try {
    ...
    const studyIds = Array.isArray(body.study_ids)
      ? body.study_ids.filter(Boolean)
      : [];

    if (!clientId || !mode) {
      return NextResponse.json(
        { error: "client_id et mode requis" },
        { status: 400 },
      );
    }
    ...
    if (studyIds.length > 0) {
      ...
      if (Number.isNaN(monthStart.getTime())) {
        return NextResponse.json(
          { error: "billing_month invalide (attendu: YYYY-MM)" },
          { status: 400 },
        );
      }
      ...
      if (error) {
        ...
        return NextResponse.json(
          { error: "Error lors de la finalisation" },
          { status: 500 },
        );
      }
      studies = data ?? [];
    ...
    if (studies.length === 0) {
      return NextResponse.json(
        { error: "No billable studies found" },
        { status: 400 },
      );
    }
    ...
    const subtotal = studies.reduce(
      (acc, study) => acc + (priceMap.get(study.study_type) ?? 0),
      0,
    );
    const tvaRate = 0;
    ...
    const bucketExists = (buckets ?? []).some(
      (bucket) => bucket.name === "invoices-files",
    );
    if (!bucketExists) {
      const { error: createBucketError } =
        await adminClient.storage.createBucket("invoices-files", {
          public: false,
        });
