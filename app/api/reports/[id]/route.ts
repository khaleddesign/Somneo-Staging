...
> {
  const [
    ...
    admin
      .from("study_reports")
      .select(
        "id, study_id, agent_id, content, status, pdf_path, created_at, validated_at, updated_at",
      )
      .eq("id", reportId)
      .maybeSingle(),
    admin
  ...
  const { data: study, error: studyError } = await admin
    .from("studies")
    .select(
      "client_id, assigned_agent_id, client:profiles!studies_client_id_fkey(institution_id)",
    )
  ...
  const institution = (
    study.client as unknown as { institution_id: string } | null
  )?.institution_id;
  const hasAccess =
    (callerProfile.role === "admin" &&
      institution === callerProfile.institution_id) ||
...
export async function PATCH(
  ...
  try {
    ...
    const admin = createAdminClient();
    const { report, error: accessError } = await getReportWithAccess(
      id,
      user.id,
      admin,
    );
