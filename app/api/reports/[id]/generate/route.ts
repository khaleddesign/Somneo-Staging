...
export async function POST(
  ...
  try {
    ...
    if (uploadError) {
      console.error(
        "[api/reports/[id]/generate] Storage upload error:",
        uploadError,
      );
    ...
    if (reportUpdateError) {
      console.error(
        "[api/reports/[id]/generate] study_reports update error:",
        reportUpdateError,
      );
      return NextResponse.json(
    ...
    if (studyUpdateError) {
      console.error(
        "[api/reports/[id]/generate] studies update error:",
        studyUpdateError,
      );
      return NextResponse.json(
    ...
    if (signedError || !signed?.signedUrl) {
      console.error(
        "[api/reports/[id]/generate] Signed URL error:",
        signedError,
      );
      return NextResponse.json(
        {
          error:
            signedError?.message || "PDF generated but signed URL unavailable",
        },
        { status: 500 },
      );
    }

    await logAudit(user.id, "generate_pdf", "report", id, {
      study_id: study.id,
    });
