...
export async function GET(req: NextRequest) {
  try {
    ...
    if (profile.role === "client") {
      ...
      const { items: agentPageStudies, nextCursor: agentNextCursor } =
        paginateResults(rawStudies, limit, "submitted_at");
    ...
    const { items: pageStudies, nextCursor } = paginateResults(
      rawStudies,
      limit,
      "submitted_at",
    );
