...
export async function POST(req: Request) {
  try {
    ...
    if (!patient_reference) {
      return NextResponse.json({ error: 'patient_reference requis' }, { status: 400 })
    }
    if (!ALLOWED_STUDY_TYPES.includes(study_type)) {
      return NextResponse.json({ error: `study_type invalide. Valeurs acceptées: ${ALLOWED_STUDY_TYPES.join(', ')}` }, { status: 400 })
    }
    if (!ALLOWED_PRIORITIES.includes(priority)) {
      return NextResponse.json({ error: 'priority invalide' }, { status: 400 })
    }
    if (!file_path || !checksum) {
      return NextResponse.json({ error: 'file_path et checksum requis' }, { status: 400 })
    }
