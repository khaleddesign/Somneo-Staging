  ...
  try {
    ...
    const { data: studies, error: studiesError } = agentIds.length
      ? await admin
          .from("studies")
          .select(
            "assigned_agent_id, status, submitted_at, completed_at, updated_at",
          )
    ...
    const mapped = (agents || []).map((agent) => {
      const mine = (studies || []).filter(
        (study) => study.assigned_agent_id === agent.id,
      );
      const enCours = mine.filter(
        (study) => study.status === "en_cours",
      ).length;
...
export async function PATCH(req: Request) {
  try {
    ...
    if (id === auth.user?.id) {
      return NextResponse.json(
        { error: "Suppression de votre propre compte impossible" },
        { status: 400 },
      );
    }
    ...
    if (typeof is_suspended === "boolean")
      updatePayload.is_suspended = is_suspended;
