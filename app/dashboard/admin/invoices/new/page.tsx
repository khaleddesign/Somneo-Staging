...
export default function AdminNewInvoicePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setClients((clientsData.clients || []).map((c: ClientRow) => ({ id: c.id, full_name: c.full_name, email: c.email })))
  ...
  const studiesForInvoice = mode === 'monthly' ? monthlyStudies : perStudySelection

  const estimatedSubtotal = useMemo(
    () => studiesForInvoice.reduce((sum, study) => sum + (priceByType[study.study_type] || 0), 0),
            ...
            {mode === 'per_study' ? (
              <div className="space-y-2">
                {eligibleStudies.map((study) => {
                  ...
                  return (
                    <label key={study.id} className="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-3 py-2">
                      <div className="flex items-center gap-3">
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={(e) => toggleStudy(study.id, e.target.checked)}
                        />
                        <div>
                          <p className="text-sm font-body text-midnight">{study.patient_reference}</p>
                          <p className="text-xs text-gray-500 font-body">{study.study_type} · {formatDate(study.completed_at || study.submitted_at)}</p>
                        </div>
                      </div>
                      <p className="text-sm font-heading text-gold tabular-nums">{formatMoney(estimatedPrice)}</p>
                    </label>
                  )
                })}
              ...
              <div className="space-y-2">
                {monthlyStudies.map((study) => {
                  const estimatedPrice = priceByType[study.study_type] || 0
                  return (
                    <div key={study.id} className="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-3 py-2">
                      <div>
                        <p className="text-sm font-body text-midnight">{study.patient_reference}</p>
                        <p className="text-xs text-gray-500 font-body">{study.study_type} · {formatDate(study.completed_at || study.submitted_at)}</p>
                      </div>
