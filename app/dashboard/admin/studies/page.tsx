  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const studiesData = studiesRes.ok
        ...
        : { studies: [] };
      const agentsData: { agents: Agent[] } = agentsRes.ok
    ...
    return studies.filter((study) => {
      ...
      const agentMatch =
        ...
        (agent === "unassigned"
          ...
          : study.assigned_agent_id === agent);
      const clientMatch = client === "all" || study.client_id === client;
      const dateMatch =
        !date ||
        new Date(study.submitted_at).toISOString().slice(0, 10) === date;
      return (
        statusMatch && priorityMatch && agentMatch && clientMatch && dateMatch
      );
    });
      ...
      current.map((study) =>
        study.id === selectedStudy.id
          ? {
              ...study,
              assigned_agent_id:
                newAgentId === "unassigned" ? null : newAgentId,
            }
          : study,
      ),
    );
      ...
      <div className="p-2 md:p-4 space-y-6 bg-[#f0f4f8]">
        <div>
          ...
          <p className="text-gray-500 font-body">
            Global overview and study reassignment
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <Input
            ...
            className="bg-[#f8fafc] border-2 border-transparent rounded-xl focus-visible:border-teal"
          />
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 overflow-x-auto">
          <h2 className="text-lg text-midnight font-heading mb-4">
            Filtered view
          </h2>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 overflow-x-auto">
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-[#fafbfc] border-b border-gray-100">
              <tr>
                <th className="px-3 py-2 text-left text-xs uppercase tracking-wider text-gray-500 font-heading">
                  Patient
                </th>
                <th className="px-3 py-2 text-left text-xs uppercase tracking-wider text-gray-500 font-heading">
                  Client
            ...
            <tbody>
              {filtered.map((study) => (
                <tr
                  key={study.id}
                  className="border-t border-gray-100 hover:bg-teal/3 transition-colors"
                >
                  ...
                  <td className="px-3 py-3 font-body">
                    {study.assigned_agent_id
                      ? agentMap.get(study.assigned_agent_id) || "—"
                      : "Unassigned"}
                  </td>
                  <td className="px-3 py-3 font-body">
