...
export default function AdminDashboardPage() {
  ...
  const cards = [
    ...
    { label: 'Total études', value: stats?.total_studies ?? 0, icon: FileText, bg: 'bg-teal/10', color: 'text-teal' },
    { label: 'Études en attente', value: stats?.en_attente_non_assignees ?? 0, icon: Clock, bg: 'bg-gold/10', color: 'text-gold' },
    { label: 'Études en cours', value: stats?.en_cours ?? 0, icon: Timer, bg: 'bg-teal/10', color: 'text-teal' },
    { label: 'Terminées ce mois', value: stats?.termine_ce_mois ?? 0, icon: CheckCircle2, bg: 'bg-gold/10', color: 'text-gold' },
    { label: 'Délai moyen', value: `${stats?.avg_turnaround ?? 0}h`, icon: Timer, bg: 'bg-teal/10', color: 'text-teal' },
  ]
              ...
              <table className="min-w-full text-sm font-body">
                ...
                <tbody>
                  ...
                  {!loading && agents.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-3 py-6 text-center text-gray-500 font-body">Aucune donnée agent</td>
                    </tr>
                  )}
                </tbody>
              </table>
