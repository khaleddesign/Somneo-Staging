...
export const StudyList: FC<StudyListProps> = ({
  ...
  const [exportPeriod, setExportPeriod] = useState<
    "selection" | "month" | "year" | "all" | "custom"
  >("selection");
  ...
  const customMonth =
    customYear && customMonthNum ? `${customYear}-${customMonthNum}` : "";

  function isSlaBreached(study: Study) {
    if (study.priority !== "high" || study.status !== "en_attente")
      return false;
  ...
  function handleExportCSV() {
    ...
    link.setAttribute(
      "download",
      `export_etudes_${exportPeriod}_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  function handleExportPDF() {
    ...
    const title =
      exportPeriod === "custom" && customMonth
    ...
    const highPriorityCount = toExport.filter(
      (s) => s.priority === "high",
    ).length;
    doc.setFillColor(...lightGray);
    ...
    // Box 3: Terminées
    const completedCount = toExport.filter(
      (s) => s.status === "termine",
    ).length;
    doc.setFillColor(...lightGray);
    ...
    autoTable(doc, {
      ...
      didDrawPage: function (data) {
        ...
        doc.text(
          "CONFIDENTIAL DOCUMENT - Protected medical data (HDS/GDPR).",
          14,
          pageHeight - 10,
        );
        ...
        doc.text(pageNumberStr, pageWidth - 14, pageHeight - 10, {
          align: "right",
        });
      },
    });
    ...
    doc.save(
      `SomnoConnect_Report_${exportPeriod}_${new Date().toISOString().split("T")[0]}.pdf`,
    );
  }
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <option value="selection">
              Export selection ({selectedIds.size})
            </option>
        ...
        <table className="min-w-full border border-gray-100 text-sm bg-white rounded-2xl overflow-hidden shadow-sm">
          <thead>
            <tr className="bg-[#fafbfc] border-b border-gray-100">
              <th className="px-3 py-3 text-left w-12 text-center">
                <Checkbox
                  checked={
                    studies.length > 0 && selectedIds.size === studies.length
                  }
              ...
              <th className="px-3 py-3 text-center text-xs text-gray-400 uppercase tracking-wider font-heading">
                Archive
              </th>
              <th className="px-3 py-3 text-left text-xs text-gray-400 uppercase tracking-wider font-heading">
                Actions
          ...
          <tbody>
            {studies.map((study) => (
              <tr
                key={study.id}
                className="border-b border-gray-100 hover:bg-teal/3 transition-colors"
              >
                ...
                <td className="px-3 py-3">
                  <div className="flex items-center gap-2">
                    <span
                      className={`px-2 py-1 rounded text-xs font-semibold ${priorityColors[study.priority]}`}
                    >
                      {study.priority}
                    </span>
                    {isSlaBreached(study) && (
                ...
                <td className="px-3 py-3">
                  <div className="flex items-center gap-2">
                    <span
                      className={`px-2 py-1 rounded text-xs font-semibold ${statusColors[study.status]}`}
                    >
                ...
                {(role === "agent" || role === "admin") && (
                  <>
                    ...
                    <td className="px-3 py-3 font-body text-sm text-gray-600">
                      {study.result_date
                        ? new Date(study.result_date).toLocaleDateString(
                            "fr-FR",
                          )
                        : "—"}
                    </td>
                ...
                <td className="px-3 py-3 text-center">
                  {study.archived_at ? (
                    <span
                      title={`Archivé le ${new Date(study.archived_at).toLocaleDateString("en-GB")}`}
                    >
