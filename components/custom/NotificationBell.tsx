...
export default function NotificationBell() {
  ...
  const notificationContent = useMemo(() => {
      ...
      return (
        <div className="space-y-2">
          {Array.from({ length: 3 }, (_, i) => (
            <div
              key={i}
              className="animate-pulse rounded-lg border border-gray-100 bg-gray-50 p-3 h-16"
            />
          ))}
        </div>
      );
    ...
    if (notifications.length === 0) {
      return (
        <p className="text-sm text-gray-500 font-body">Aucune notification</p>
      );
    }
    ...
    return (
      <div className="space-y-3 max-h-80 overflow-auto pr-1">
        {notifications.map((notification) => (
          ...
          >
            <p className="text-sm text-midnight font-heading">
              {notification.title}
            </p>
