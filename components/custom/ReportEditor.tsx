...
export default function ReportEditor({
  ...
  const saveReport = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Error de sauvegarde";
      setError(message);
      throw err;
    } finally {
      setSaving(false);
  ...
  const generatePdf = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Error de génération PDF";
  ...
  const generateAutoDraft = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !payload.report) {
        throw new Error(
          payload.error || "Impossible de générer le brouillon médical",
        );
      }
      ...
      const message =
        err instanceof Error ? err.message : "Error de génération du brouillon";
    ...
    async function loadData() {
      ...
      try {
        const reportRes = await fetch(
          `/api/reports?study_id=${encodeURIComponent(studyId)}`,
        );
        ...
        if (reportRes.ok) {
          ...
          if (!createRes.ok) {
            const message =
              isObject(createPayload) && typeof createPayload.error === "string"
                ? createPayload.error
        ...
        if (!templateRes.ok) {
          const message =
            isObject(templatePayload) &&
            typeof templatePayload.error === "string"
              ? templatePayload.error
              : "Impossible de charger le template";
          throw new Error(message);
        }

        const loadedTemplate = (templatePayload as TemplateGetResponse)
          .template;
        if (!mounted) return;
  ...
  return (
    <div className="space-y-6 rounded-xl border border-gray-100 bg-white p-6">
            ...
            {section.type === "text" && (
              <div className="space-y-3">
                <Textarea
                  ...
                  onChange={(e) =>
                    updateValue(section.id, "text", e.target.value)
                  }
                  ...
                  <label key={field.key} className="space-y-1">
                    ...
                    {selectFieldOptions[field.key] ? (
                      <select
                        ...
                        onChange={(e) =>
                          updateValue(section.id, field.key, e.target.value)
                        }
            ...
            {section.type === "richtext" && (
              <div className="space-y-3">
                <Textarea
                  ...
                  onChange={(e) =>
                    updateValue(section.id, "richtext", e.target.value)
                  }
      ...
      <div className="space-y-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <p className="font-heading text-midnight">
          Extraction auto (copier-coller brut)
        </p>
        <Textarea
